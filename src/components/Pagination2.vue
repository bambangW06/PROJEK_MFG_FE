    ...
    <CCol md="3" v-if="parseInt(this.props.page.totalPages) > 0">
      <CPagination align="end">
        ...
        <CPaginationItem
          aria-label="Previous"
          href="#"
          :disabled="this.props.page.pageNo > 1 ? false : true"
          @click.prevent="doDoSearch(1)"
        >
          <span aria-hidden="true">&laquo;</span>
        </CPaginationItem>
        <CPaginationItem
          v-if="this.arrayPages[0] > 0"
          :active="this.arrayPages[0] == this.props.page.pageNo ? true : false"
          href="#"
          @click.prevent="doDoSearch(parseInt(this.arrayPages[0]))"
          >{{ this.arrayPages[0] }}</CPaginationItem
        >
        <CPaginationItem
          v-if="
            this.arrayPages[1] >= 1 &&
            this.arrayPages[1] <= parseInt(this.props.page.totalPages)
          "
          :active="this.arrayPages[1] == this.props.page.pageNo ? true : false"
          href="#"
          @click.prevent="doDoSearch(parseInt(this.arrayPages[1]))"
          >{{ this.arrayPages[1] }}</CPaginationItem
        >
        <CPaginationItem
          v-if="
            this.arrayPages[2] >= 1 &&
            this.arrayPages[2] <= parseInt(this.props.page.totalPages)
          "
          :active="this.arrayPages[2] == this.props.page.pageNo ? true : false"
          href="#"
          @click.prevent="doDoSearch(parseInt(this.arrayPages[2]))"
          >{{ this.arrayPages[2] }}</CPaginationItem
        >
        <CPaginationItem
          v-if="this.arrayPages[3] <= parseInt(this.props.page.totalPages)"
          :active="this.arrayPages[3] == this.props.page.pageNo ? true : false"
          href="#"
          @click.prevent="doDoSearch(parseInt(this.arrayPages[3]))"
          >{{ this.arrayPages[3] }}</CPaginationItem
        >
        <CPaginationItem
          v-if="this.arrayPages[4] <= parseInt(this.props.page.totalPages)"
          :active="this.arrayPages[4] == this.props.page.pageNo ? true : false"
          href="#"
          @click.prevent="doDoSearch(parseInt(this.arrayPages[4]))"
          >{{ this.arrayPages[4] }}</CPaginationItem
        >
        <CPaginationItem
          aria-label="Next"
          href="#"
          :disabled="
            this.props.page.pageNo < this.props.page.totalPages ? false : true
          "
...
export default {
  ...
  methods: {
    async changeRowPerPage(event) {
      let firstRowInPage =
        (this.props.page.pageNo - 1) * this.props.page.pageSize + 1
      let targetPage = parseInt(firstRowInPage / event.target.value)
    ...
    doDoSearch(page) {
      ...
      this.props.page.pageSize = this.row_per_page
    ...
    setPage() {
      if (parseInt(this.props.page.pageNo) === 1) {
      ...
      } else if (parseInt(this.props.page.pageNo) === 2) {
        ...
        parseInt(this.props.page.pageNo) ===
        parseInt(this.props.page.totalPages) - 1
      ) {
        ...
        parseInt(this.props.page.pageNo) > 0 &&
        parseInt(this.props.page.pageNo) ===
          parseInt(this.props.page.totalPages)
      ) {
  ...
  created() {
    this.row_per_page = this.props.page.pageSize
    this.setPage()
  },
  setup() {
    return {}
