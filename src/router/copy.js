...
import store from '@/store' // ⬅️ Tambahkan ini di atas
...
const routes = [
  {
    ...
    redirect:
      ...
      (!localStorage.id_token || localStorage.id_token == '')
        ? () => {
            window.location.href = process.env.dc + '/login'
    ...
    children: [
      ...
      {
        path: '/login', // ⬅️ path halaman login
        name: 'Login',
        component: () => import('@/views/LoginPage.vue'), // ⬅️ view login.vue
      },
